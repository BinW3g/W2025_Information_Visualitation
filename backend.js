...
const dropdownData = {
  countries: [
    ...
    "New Zealand", "South Africa", "USA", "Russia", "Sweden"
  ],
  scores: [
...
function init() {
  ...
  // d3.csv("data/openpowerlifting-2025-11-22-823f23d6.csv").then(data => {
  ...
  //   });
    ...
    // 4. Initial Filter Application (optional)
    // applyFilters();
...
function applyFilters() {
  ...
  const filters = {
    country: elements.countrySelect.value, // "all" or specific
  ...
  const filteredData = globalData.filter(d => {
    ...
    if (filters.country !== "all" && d.Country !== filters.country && d.MeetCountry !== filters.country) return false;
    if (filters.gender !== "all" && d.Sex !== filters.gender) return false;
    if (filters.equipment !== "all" && d.Equipment !== filters.equipment) return false;
    if (filters.event !== "all" && d.Event !== filters.event) return false;

    // 2. Boolean Filters
    if (filters.isTested && !d.Tested) return false;
    ...
    if (d.Age !== null && (d.Age < filters.ageMin || d.Age > filters.ageMax)) return false;

    // Handle Bodyweight
    if (d.BodyweightKg !== null && (d.BodyweightKg < filters.weightMin || d.BodyweightKg > filters.weightMax)) return false;

    // Handle Year
    if (d.Year !== null && (d.Year < filters.yearMin || d.Year > filters.yearMax)) return false;
  ...
  console.log(`Matched ${filteredData.length} of ${globalData.length} records.`);

  // Here you would trigger the Map Update or Chart Update passing 'filteredData'
}
